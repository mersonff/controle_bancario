...
RSpec.feature "Withdraws", type: :feature do
  before do
  ...
  scenario "Should withdraw a value from the account" do
    agency = Agency.first
    account = Account.create!(account_number: 001, balance: 3000, limit: 1000, agency_id: agency.id )

    visit(new_transaction_path)
    select "1", :from => "transaction_account_id"
    fill_in "Valor", with: 1000
    select "Saque", :from => "transaction_type_of_transaction"
    click_button "Salvar"
